        ...
        public async Task ProcessAsync(MessagePayload messagePayload, AgentContext agentContext)
        {
            switch (messagePayload.GetMessageType())
            {
                case MessageTypes.ConnectionInvitation:
                    ...
                    await _connectionService.AcceptInvitationAsync(agentContext.Wallet, invitation);
                    return;

                case MessageTypes.ConnectionRequest:
                    var request = messagePayload.GetMessage<ConnectionRequestMessage>();
                    var connectionId = await _connectionService.ProcessRequestAsync(
                        agentContext.Wallet, request, agentContext.Connection);
                ...
                case MessageTypes.ConnectionResponse:
                    ...
                    await _connectionService.ProcessResponseAsync(agentContext.Wallet, response, agentContext.Connection);
                    return;
